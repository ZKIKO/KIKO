...
elseif redis:sismember(vzaec..':MONSHA_BOT:'..arg.ChatID,UserID) then 
...
elseif redis:sismember(vzaec..':MONSHA_BOT:'..msg.chat_id_,UserID) then 
...
if redis:sismember(vzaec..':MONSHA_BOT:'..arg.ChatID,arg.UserID) then 
  return sendMsg(arg.ChatID,arg.MsgID,"📮 » تم بالتأكيد رفع  ❲ "..NameUser.." ❳  منشئ في المجموعة \n✓") 
else
redis:hset(vzaec..'username:'..arg.UserID,'username',ReUsername)
redis:sadd(vzaec..':MONSHA_BOT:'..arg.ChatID,arg.UserID)
...
UserName = arg.UserName
if redis:sismember(vzaec..':MONSHA_BOT:'..arg.ChatID,UserID) then 
  return sendMsg(arg.ChatID,arg.MsgID,"📮 » تم بالتأكيد رفع  ❲ "..NameUser.." ❳  منشئ في المجموعة \n✓") 
else
redis:hset(vzaec..'username:'..UserID,'username',UserName)
redis:sadd(vzaec..':MONSHA_BOT:'..arg.ChatID,UserID)
return sendMsg(arg.ChatID,arg.MsgID,"📮 » تم رفع  ❲ "..NameUser.." ❳  منشئ في المجموعة \n✓") 
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
if not redis:sismember(vzaec..':MONSHA_BOT:'..arg.ChatID,arg.UserID) then
return sendMsg(arg.ChatID,arg.MsgID,"📮 » تم بالتأكيد تنزيل  ❲ "..NameUser.." ❳  منشئ في المجموعة \n✓") 
else
redis:srem(vzaec..':MONSHA_BOT:'..arg.ChatID,arg.UserID)
...
NameUser = Hyper_Link_Name(data)
UserName = Flter_Markdown(arg.UserName)
if not redis:sismember(vzaec..':MONSHA_BOT:'..arg.ChatID,UserID) then 
  return sendMsg(arg.ChatID,arg.MsgID,"📮 » تم بالتأكيد تنزيل  ❲ "..NameUser.." ❳  منشئ في المجموعة \n✓") 
else
redis:srem(vzaec..':MONSHA_BOT:'..arg.ChatID,UserID)
return sendMsg(arg.ChatID,arg.MsgID,"📮 » تم تنزيل  ❲ "..NameUser.." ❳  منشئ في المجموعة \n✓") 
end
end,{ChatID=msg.chat_id_,MsgID=msg.id_,UserName=MsgText[2]})
elseif MsgText[2] and MsgText[2]:match('^%d+$') then
...
if not msg.SudoUser then return "📛*¦* هذا الامر يخص {المطور} فقط  \n🚶" end
local NumMnsha = redis:scard(vzaec..':MONSHA_BOT:'..msg.chat_id_)
if NumMnsha ==0 then 
return "📛¦ عذرا لا يوجد منشئيين ليتم مسحهم \n!" 
end
redis:del(vzaec..':MONSHA_BOT:'..msg.chat_id_)
...
kick_user(v.user_id_,arg.ChatID,function(arg,data)  
redis:srem(vzaec..':MONSHA_BOT:'..arg.ChatID,v.user_id_)
...
Getrtb = 'مطور ' 
elseif redis:sismember(vzaec..':MONSHA_BOT:'..arg.ChatID,arg.UserID) then
Getrtb = 'منشئ' 
...
if MsgText[1] == "تنزيل الكل" then
  if not msg.Admin then return "📛*¦* هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n🚶" end
    ...
    if redis:sismember(vzaec..':MONSHA_BOT:'..msg.chat_id_,UserID) then 
...
huk = false
    if msg.SudoBase then 
      ...
      redis:srem(vzaec..':MONSHA_BOT:'..msg.chat_id_,UserID)
...
local inline = {{{text="قنآهہ‏‏ آلسـورس : النيزك 🍃",url="t.me/vzaec"}}}
send_key(msg.sender_user_id_,'  [قناة السورس : النيزك](t.me/vzaec)',nil,inline,msg.id_)
return false
end
...
if msg.adduser == SUDO_ID then 
  ...
  gtupe = 'منشئ اساسي'
  elseif msg.GroupActive and redis:sismember(vzaec..':MONSHA_BOT:'..msg.chat_id_,msg.adduser) then 
  gtupe = 'المنشىء'
