...
local GetVerisonnizks = https.request('https://raw.githubusercontent.com/th3nk/nizksFile/master/GetVersion.txt') or 0
print('\n\27[1;36m￤تم آدخآل آيدي آلمـطـور بنجآح , سـوف يتم تشـغيل آلسـورس آلآن .\n￤Success Save USERID : \27[0;32m['..SUDO_USER..']\n\27[0;39;49m')
nizks = Token:match("(%d+)")
redis:mset(
nizks..":VERSION",GetVerisonnizks,
nizks..":SUDO_ID:",SUDO_USER,
nizks..":DataCenter:",'Amsterdam',
nizks..":UserNameBot:",BOT_User,
nizks..":ApiSource",'http://th3nezk.aba.vg/',
nizks..":NameBot:","نيزك",
"TH3nizks_INSTALL","Yes"
)
redis:hset(nizks..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
info = {} 
...
nizks = Token:match("(%d+)")
our_id = tonumber(nizks)
Apinizks = redis:get(nizks..":ApiSource")
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(nizks..":UserNameBot:")
SUDO_ID = tonumber(redis:get(nizks..":SUDO_ID:"))
if not SUDO_ID then io.popen("rm -fr ./inc/Token.txt") end
SUDO_USER = redis:hgetall(nizks..'username:'..SUDO_ID).username
version = redis:get(nizks..":VERSION")
DataCenter = redis:get(nizks..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..nizks..')\027[0;32m\n'
...
..'¦ Api_Src->: \27[1;34m'..Apinizks..'\027[0;32m\n'
...
if not redis:get(nizks..":Check_Bot:"..W) then
Rgz,res=http.request(Apinizks..Tkml..W)
if res == 200 and Rgz == "SendMsg" then redis:setex(nizks..":Check_Bot:"..W,1800,true) return false else return Rgz end 
end 
end
...
if msg.type == "pv" and redis:get(nizks..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end
if msg.type ~= "pv" and redis:get(nizks..'sender:'..msg.sender_user_id_..':'..msg.chat_id_..'flood') then
print("\27[1;31mThis Flood Sender ...\27[0")
Del_msg(msg.chat_id_,msg.id_)
return false
end
if redis:get(nizks..'group:add'..msg.chat_id_) then 
...
if msg.GroupActive and redis:get(nizks..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
end
return false 
end
...
msg.Rank = 1
elseif redis:sismember(nizks..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت 👨🏽‍💻'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(nizks..':MONSHA_Group:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'منشئ اساسي'
msg.TheRank = 'منشئ اساسي 👲🏼'
msg.Rank = 11
elseif msg.GroupActive and redis:sismember(nizks..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء 👷🏽'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(nizks..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت 👨🏼‍⚕️' 
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(nizks..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت 👨🏼‍🎓'
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(nizks..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(nizks..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(nizks..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(nizks..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(nizks..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(nizks..':WELCOME_BOT'),[[💯¦ مـرحبآ آنآ بوت آسـمـي ]]..redis:get(nizks..':NameBot:')..[[ 🎖
...
if not msg.Special and not msg.Admin and redis:get(nizks.."lock_Add"..msg.chat_id_) then
...
if msg.Special or msg.Admin then redis:srem(nizks..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return false end
...
if msg.Special then redis:srem(nizks..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(nizks..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.nizks then 
if msg.text and ScriptFile.inizks then
Xnizks = ScriptFile.nizks
local list = redis:hgetall(nizks..":AwamerBotArray:"..msg.chat_id_)
for nizks2,k in pairs(list) do
Text = msg.text
Text2 = k
if Text:match(nizks2) then 
local amrr = {Text:match(nizks2)}
local AmrOld = redis:hgetall(nizks..":AwamerBotArray2:"..msg.chat_id_)
amrnew = "" amrold = ""
...
GetMsg = ScriptFile.inizks(msg,{Text:match(Text2)})
if GetMsg then
...
Text = Text:gsub("ک","ك")
Text = Text:gsub("ه‍","ه")
if Text:match(nizks) then -- Check Commands To admin
AF = CheckBotA(msg) if AF then return sendMsg(msg.chat_id_,msg.id_,AF) end 
GetMsg = ScriptFile.inizks(msg,{Text:match(nizks)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",nizks.." | Plugin is: \27[1;32mScript.lua\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
return false
end 
end
...
if ScriptFile.dnizks then
if ScriptFile.dnizks(msg) == false then
return false
end
...
if Plug.nizks then 
if msg.text and Plug.inizks then
for k, nizks in pairs(Plug.nizks) do
if msg.text:match(nizks) then
local GetMsg = Plug.inizks(msg,{msg.text:match(nizks)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",nizks.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source nizks")
end 

end
else
print("The File Script.lua Not Runing in The Source nizks")
end
end
...
function tdcli_update_callback(data)
	...
	redis:srem(nizks..'users',msg.sender_user_id_)
	end
...
if redis:get(nizks..":propin"..msg.chat_id_) == msg.content_.text_ then
redis:del(nizks..":propin"..msg.chat_id_)
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,d) vardump(d)end,nil)   
	...
	if redis:get(nizks..'group:add'..data.chat_.id_) then
	redis:set(nizks..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	elseif data.ID == "UpdateChannel" then  
	if data.channel_.status_.ID == "ChatMemberStatusKicked" then 
	if redis:get(nizks..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(nizks..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(nizks..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	nizks..'group:name'..k,IDS.Title,
	nizks..'num_msg_max'..k,5,
	nizks..'group:add'..k,true,
	nizks..'lock_link'..k,true,
	nizks..'lock_spam'..k,true,
	nizks..'lock_webpage'..k,true,
	nizks..'lock_markdown'..k,true,
	nizks..'lock_flood'..k,true,
	nizks..'lock_bots'..k,true,
	nizks..'mute_forward'..k,true,
	nizks..'mute_contact'..k,true,
	nizks..'mute_document'..k,true,
	nizks..'mute_inline'..k,true,
	nizks..'lock_username'..k,true,
	nizks..'replay'..k,true
	)
	redis:sadd(nizks..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(nizks..'username:'..ID,'username',user)
	redis:sadd(nizks..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(nizks..'username:'..ID,'username',user)
	redis:sadd(nizks..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(nizks..'username:'..ID,'username',user)
	redis:sadd(nizks..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'📦*¦* تم رفع آلنسـخهہ‏‏ آلآحتيآطـيهہ\n⚖️*¦* حآليآ عدد مـجمـوعآتگ هہ‏‏يهہ‏‏ *'..redis:scard(nizks..'group:ids')..'* 🌿\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(nizks..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(nizks..'username:'..data.user_.id_).username
	...
	redis:hset(nizks..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
